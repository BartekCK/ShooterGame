...
{
	Path path;
	return new Enemy(path.ENEMY_SMALL_SOLDIER, getRandX(player),getRandY(), 6, 2, 50, player);
}

Enemy* BuildEnemy::getSoldier(Player* player)
{
	Path path;
	return new Enemy(path.ENEMY_SOLDIER, getRandX(player), getRandY(), 10, 2, 50, player);
}

Enemy* BuildEnemy::getGalacticSoldier(Player* player)
{
	Path path;
	return new Enemy(path.ENEMY_GALACTIC_SOLDIER, getRandX(player), getRandY(), 17, 2, 50, player);
}

Enemy* BuildEnemy::getZombie(Player* player)
{
	Path path;
	return new Enemy(path.ENEMY_ZOMBIE, getRandX(player), getRandY(), 19, 2, 50, player);
}

Enemy* BuildEnemy::getFastZombie(Player* player)
{
	Path path;
	return new Enemy(path.ENEMY_SMALL_ZOMBIE, getRandX(player), getRandY(), 8, 2, 50, player);
}
